...
Program Strain
! AS_Calc_v2 calculates the strain field with data provided by AS_LBFG_v2 in: half-torus, cone, truncated cone, pyramidal, core-shell quantum dots (QDs) heterostructures (QD=1,2,21,3,4). The QDs are of type AB/AC (three types of atoms A, B, C) for all QD shapes and A/B (two types of atoms A, B) for core-shell. In the comments below GaSb/GaAs and Si/C are taken as examples.
! One needs the files: fort.111, fort.124, fort.201, temp280.dat, temp240, temp260, and fort.601 or fort.6ab0 (e.g., fort.6010, fort.6120, ...) generated by code AS_LBFG_v2.  AS_LBFG_v2 also generates files of the atomic positions: fort.38 (for Ga IN),fort.39 (for Ga OUT),fort.140 (for Ga),fort.160(for Sb),fort.180(for As) if GaSb/GaAs is taken as an example.
!
    use DataType
    implicit none
...
!
!------------------------------
!reading the relaxed atoms coords generated by code AS_LBFG_v2:
!rewind(6330)
rewind(601)
do i=1,3*Nt,3
...
!
     if((rp(m,3).ge.adf(index)).and.(ff.ne.0)) then ! forces the damping of epsxx, epsyy, epszz.
...
!======================================================================
! CELL for the half-torus analysis (radius < nxp*a1)
...
!
!======================================================================
!In a square(for pyramid) or disk(for cone, truncated cone, half-torus, core-shell) of radius nxp*a1:
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
if (counter.eq.0) then
   ...
   print*, 'radcut=',radcut,'Angstrom'
   if (met==1) then
       print*,'Strain field is calculated by Pryor method for xy-plane and x,y,z directions.'
   end if
!
   if (met==2) then
       print*,'Strain field is calculated by Gullet method for the xy-plane and x, y, z directions.'
   end if
!
    end if
!
...
!  
    ...
    !QDHTF-quantum dot half-torus function
    !Generates half-torus function in Cartesian coords
...
Subroutine QDConeTruncF(x,y,z,log)
    ...
    !Rc-cone radius; h-cone height; htc-height of truncated cone
use DataType
...
Logical:: log
    ...
    if (htc.ge.h) then
    write (6,61)
61  format ('The height of truncated cone htc can not be larger than its height. Change htc in code AS_LBFG_v2')
    STOP
    end if
!continue
